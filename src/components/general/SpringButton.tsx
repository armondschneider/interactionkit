...
export function SpringButton({ children, className = "", pressSpring = true, ...props }: ButtonProps) {
  ...
  const interactionClassName = pressSpring
    ? "transition-colors duration-150 hover:bg-neutral-700 dark:hover:bg-white"
  ...
  return (
    <motion.button
      ...
      className={`rounded-full bg-neutral-800 px-4 py-2 text-sm font-medium text-white focus:outline-none focus-visible:ring-2 focus-visible:ring-neutral-400 focus-visible:ring-offset-2 dark:bg-neutral-200 dark:text-neutral-900 dark:focus-visible:ring-neutral-500 dark:focus-visible:ring-offset-neutral-900 ${interactionClassName} ${className}`}
    >
      {children}
    </motion.button>
  );
